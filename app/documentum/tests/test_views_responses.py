...
class DocumentumViewResponsesTests(TestCase):
    ...
    def test_category_list_returns_200(self):
        url = reverse('docs:category_list')
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        # Ensure template used is the docs/category_list.html
        self.assertTemplateUsed(resp, 'docs/category_list.html')

    def test_document_list_returns_200(self):
        url = reverse('docs:document_list', args=[self.cat.slug])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'docs/document_list.html')

    def test_document_detail_returns_200(self):
        url = reverse('docs:document_detail', args=[self.cat.slug, self.doc.slug])
        resp = self.client.get(url)
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, 'docs/document_detail.html')
